...
def edges_to_jaeger(
    ...
    processes = {f"p{i}": {"serviceName": svc} for i, svc in enumerate(sorted(services))}
...
def main():
    ...
    parser.add_argument(
        "--scale", type=int, default=100, help="Edge weight multiplier (default: 100)"
    )
    ...
    convert(
        app_dir, args.output or Path(f"benchmarks/real_traces/{args.app}.json"), scale=args.scale
    )
