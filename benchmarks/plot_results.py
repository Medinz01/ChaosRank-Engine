...
def discovery_curve(
    ...
    for row in rows:
        ...
        per_trial_curves.append(
            [sum(1 for f in found_at if f <= x) for x in range(1, n_services + 1)]
        )
...
def plot(chaosrank_rows: list, random_rows: list, n_services: int) -> None:
    ...
    ax.plot(
        x, rnd_mean, color="#e74c3c", linewidth=2.5, label="Random selection (baseline)", zorder=3
    )
    ...
    rnd_all_found_mean = next(
        (i + 1 for i, v in enumerate(rnd_mean) if v >= N_WEAKNESSES - 0.1), n_services
    )
