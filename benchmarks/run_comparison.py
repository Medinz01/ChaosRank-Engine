...
def run_benchmark() -> dict:
    ...
    with open(RESULTS_CSV, "w", newline="") as f:
        writer = csv.DictWriter(
            ...
            fieldnames=[
                "trial",
                "strategy",
                "experiments_to_first",
                "experiments_to_all",
                "uploadcreator_found_at",
                "uploadmedia_found_at",
                "urlservice_found_at",
            ],
        )
        writer.writeheader()
    ...
    cr_first_mean = (
        sum(r["experiments_to_first"] for r in rows if r["strategy"] == "chaosrank") / N_TRIALS
    )
    rnd_first_mean = (
        sum(r["experiments_to_first"] for r in rows if r["strategy"] == "random") / N_TRIALS
    )
    ...
    print(f"{'-' * 60}")
    print(
        f"{'Mean experiments to FIRST weakness':<40} {cr_first_mean:>10.1f} {rnd_first_mean:>10.1f}"
    )
