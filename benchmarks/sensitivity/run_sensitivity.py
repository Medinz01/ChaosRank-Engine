...
from chaosrank_engine.graph.builder import build_graph
...
def _write_plot(rows: list[dict], x_key: str, title: str, path: Path) -> None:
    ...
    ax.axvline(
        baseline_x, color="#16a34a", linestyle=":", linewidth=1, label=f"default ({baseline_x})"
    )
    ax.set_xlabel(x_key)
...
def _write_summary(
    ...
    stable_range_alpha = (
        f"[{min(stable_alphas):.2f}, {max(stable_alphas):.2f}]" if stable_alphas else "none"
    )
    lines += [
        "",
    ...
    stable_range_wpr = (
        f"[{min(stable_wprs):.2f}, {max(stable_wprs):.2f}]" if stable_wprs else "none"
    )
    lines += [
        "",
...
def main() -> None:
    ...
    print(
        f"  {sum(len(v.incidents) for v in service_incidents.values())} incidents across {len(service_incidents)} services"
    )
    print()
