"""Incident data model and CSV parser for the ChaosRank Engine.
Provides structures for representing and loading historical service incidents.
"""
import csv
import logging
...
@dataclass
class Incident:
    """Represents a single service incident record."""
    timestamp: datetime
    service: str
    type: str
    severity: str
...
class ServiceIncidents:
    """Container for all incidents associated with a specific service."""
    service: str
...
def parse_incidents(
    ...
    """Parse a CSV file of incidents into a service-keyed dictionary."""
            ...
            try:
                ...
                if raw_vol:
                    try:
                        ...
                        logger.warning(
                            "Row %d: invalid request_volume '%s', treating as None",
                            row_num,
                            raw_vol,
                        )
...
def _parse_timestamp(raw: str) -> datetime:
    """Attempt to parse a variety of ISO-like timestamp formats."""
    formats = [
        "%Y-%m-%dT%H:%M:%SZ",
        "%Y-%m-%dT%H:%M:%S",
        "%Y-%m-%d %H:%M:%S",
