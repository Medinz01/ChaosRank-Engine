...
def _baseline_volume(service_incidents: dict[str, ServiceIncidents]) -> float:
    """Calculate the median request volume across all services as a baseline."""
    means = []
...
def _burst_window_minutes(
    ...
    """Calculate a traffic-adaptive window for incident deduplication."""
...
def _deduplicate(
    ...
    """Remove redundant incidents within the same adaptive burst window."""
    if not incidents:
        return []
...
def _weighted_incident(
    ...
    """Calculate the normalized weight of a single incident."""
