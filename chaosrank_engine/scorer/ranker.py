"""Core ranking logic for ChaosRank.
Combines blast radius and fragility signals into a unified risk score.
"""
import logging

from chaosrank_engine.parser.incidents import ServiceIncidents
from chaosrank_engine.scorer.fragility import compute_fragility
from chaosrank_engine.scorer.suggest import suggest_fault
...
def rank_services(
    ...
    """Rank services by a weighted blend of blast radius and fragility."""
...
def _check_signal_alignment(ranked: list[dict]) -> None:
    """Detect and log misalignment between blast radius and fragility signals."""
    if len(ranked) < 4:
        return

    br_ranks = {
        r["service"]: i
        for i, r in enumerate(sorted(ranked, key=lambda r: r["blast_radius"], reverse=True))
    }
    fr_ranks = {
        r["service"]: i
        for i, r in enumerate(sorted(ranked, key=lambda r: r["fragility"], reverse=True))
    }
