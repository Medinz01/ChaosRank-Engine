"""Unit tests for the core blast radius scoring algorithm.
Verifies that the PageRank-based risk propagation correctly identifies critical
service dependencies across various graph topologies.
"""
...
class TestCalleeModel:
    def test_service_with_more_callers_scores_higher(self):
        ...
        assert scores["payment-service"] > scores["auth-service"], (
            f"payment-service (3 callers) should outscore auth-service (1 caller). Scores: {scores}"
        )
    ...
    def test_shared_dependency_scores_highest(self):
        ...
        assert scores["payment-service"] == max(scores.values()), (
            f"payment-service (5 callers) should have highest blast radius. Scores: {scores}"
        )
    ...
    def test_pure_callee_beats_pure_caller(self):
        ...
        for callee in ["payment-service", "auth-service", "cart-service"]:
            assert scores[callee] > scores["frontend"], (
                f"{callee} (has callers) should outscore frontend (no callers). Scores: {scores}"
            )
...
class TestDeepNarrowChain:
    ...
    def test_chain_with_multiple_entry_points(self):
        ...
        assert scores["X"] == max(scores.values()), (
            f"X (2 callers + chain position) should have highest blast radius. Scores: {scores}"
        )
...
class TestBlendWeights:
    ...
    def test_outdegree_heavy_surfaces_most_called_service(self):
        ...
        assert scores["payment-service"] == max(scores.values()), (
            f"payment-service (5 callers) should score highest with w_od=0.8. Scores: {scores}"
        )
...
class TestEdgeCases:
    ...
    def test_isolated_node_scores_at_most_connected_nodes(self):
        ...
        assert scores["payment-service"] > scores["isolated"], (
            f"payment-service (has callers) should outscore isolated node. Scores: {scores}"
        )
