"""Unit tests for the fragility scoring component.
Verifies that incident-based risk amplification correctly accounts for traffic
normalization, severity weighting, and temporal decay.
"""
...
from chaosrank_engine.scorer.fragility import (
    DEFAULT_LAMBDA,
...
class TestWeightedIncident:
    def test_high_traffic_incident_discounted(self):
        weights = {"high": 0.602}
        w_low = _weighted_incident(
            make_incident("svc", severity="high", request_volume=100.0), weights, None
        )
        w_high = _weighted_incident(
            make_incident("svc", severity="high", request_volume=10000.0), weights, None
        )
        assert w_low > w_high

    def test_higher_severity_scores_higher_same_traffic(self):
        weights = {"critical": 1.000, "high": 0.602}
        w_crit = _weighted_incident(
            make_incident("svc", severity="critical", request_volume=1000.0), weights, None
        )
        w_high = _weighted_incident(
            make_incident("svc", severity="high", request_volume=1000.0), weights, None
        )
...
class TestFragilityPreservation:
    ...
    def _build_scenarios(self):
        frontend_incidents = [
            make_incident(
                "frontend", type="error", severity="high", days_ago=5.0, request_volume=10000.0
            ),
            make_incident(
                "frontend", type="error", severity="high", days_ago=10.0, request_volume=10000.0
            ),
        ]
        payment_incidents = [
            make_incident(
...
class TestColdStart:
    ...
    def test_services_in_graph_not_in_incidents_get_zero_raw(self):
        si = {
            "payment-service": make_service_incidents(
                "payment-service",
                [
                    make_incident(
                        "payment-service", severity="critical", days_ago=2.0, request_volume=1000.0
                    ),
                ],
            )
        }
