"""Unit tests for the integrated service ranker.
Verifies the final risk score calculation, rank ordering, and fault suggestion
logic based on combined blast radius and fragility signals.
"""
...
class TestRiskScoreMath:
    def test_risk_is_weighted_sum(self):
        ...
        assert top["risk"] == pytest.approx(
            0.6 * top["blast_radius"] + 0.4 * top["fragility"], abs=1e-3
        )
    ...
    def test_alpha_beta_boundary_pure_blast(self):
        ranked = rank_services(
            make_blast(["a", "b", "c"], [0.9, 0.5, 0.1]), {}, alpha=1.0, beta=0.0
        )
...
class TestFaultSuggestion:
    def test_error_incidents_suggest_partial_response(self):
        ...
        assert (
            rank_services(make_blast(["svc"], [0.5]), si)[0]["suggested_fault"]
            == "partial-response"
        )
